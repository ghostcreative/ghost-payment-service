...
class AuthorizeNetService extends AbstractGhostPaymentService {
      ...
      return new Promise((resolve, reject) => {
        ...
        this._authorizeCIM.createCustomerPaymentProfile({ customerProfileId: data.customerId, paymentProfile }, (err, response) => {
          if (err) reject(err);
            ...
            paymentProfile.customerProfileId = data.customerId;
            paymentProfile.customerPaymentProfileId = response.customerPaymentProfileId;
            resolve(paymentProfile);
          }
        });
      })
      .then(card => this.getCard(_.pick(card, ['customerProfileId', 'customerPaymentProfileId'])))
    })
  }
  ...
  createCustomer (data = {}) {
    ...
    .then(customer => this.getCustomer(customer.customerProfileId))
  }

  /**
   * @param {Object} params
   * @param {String} params.customerProfileId - Authorize.net customer ID
   * @param {String} params.customerPaymentProfileId - card ID
   * @return Promise
   */
  getCard (params = {}) {
    return new Promise((resolve, reject) => {
      this._authorizeCIM.getCustomerPaymentProfile(params, (err, response) => {
        if (err) reject(err);
    ...
    .then(customer => {
      ...
      return _.map(paymentProfiles, paymentProfile => {
        return {
          ...
          billTo: _.get(paymentProfile, 'billTo', {})
        };
      });
    });
  }
